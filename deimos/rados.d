...
enum CEPH_OSD_TMAP_HDR = 'h';
enum CEPH_OSD_TMAP_SET = 's';
enum CEPH_OSD_TMAP_CREATE = 'c';
enum CEPH_OSD_TMAP_RM = 'r';

enum LIBRADOS_VER_MAJOR = 0;
enum LIBRADOS_VER_MINOR = 69;
enum LIBRADOS_VER_EXTRA = 1;

extern (D) auto LIBRADOS_VERSION(T0, T1, T2)(auto ref T0 maj, auto ref T1 min, auto ref T2 extra)
{
    return (maj << 16) + (min << 8) + extra;
}

enum LIBRADOS_VERSION_CODE = LIBRADOS_VERSION(LIBRADOS_VER_MAJOR, LIBRADOS_VER_MINOR, LIBRADOS_VER_EXTRA);

enum LIBRADOS_SUPPORTS_WATCH = 1;

/* RADOS lock flags
 * They are also defined in cls_lock_types.h. Keep them in sync!
 */
enum LIBRADOS_LOCK_FLAG_RENEW = 0x1;

/*
 * Constants for rados_write_op_create().
 */
enum LIBRADOS_CREATE_EXCLUSIVE = 1;
enum LIBRADOS_CREATE_IDEMPOTENT = 0;
 ...
 * snap id contants
 */
enum LIBRADOS_SNAP_HEAD = cast(ulong) -2;
enum LIBRADOS_SNAP_DIR = cast(ulong) -1;
 ...
 */
alias void* rados_object_list_cursor;
 ...
 * rados_getxattrs_end().
 */
alias void* rados_xattrs_iter_t;
...
struct rados_pool_stat_t
{
    ...
    ulong num_object_clones;
    /// num_objects * num_replicas
    ulong num_object_copies;
    ulong num_objects_missing_on_primary;
    /// number of objects found on no OSDs
    ulong num_objects_unfound;
    /// number of objects replicated fewer times than they should be
    /// (but found on at least one OSD)
    ulong num_objects_degraded;
    ulong num_rd;
    ulong num_rd_kb;
    ulong num_wr;
    ulong num_wr_kb;
 ...
 * @returns length of the buffer we would need to list all pools
 */
int rados_inconsistent_pg_list (
    rados_t cluster,
    long pool,
    char* buf,
    size_t len);
 ...
 * @returns 0 on success, negative error code on failure
 */
int rados_pool_create_with_auid (
    rados_t cluster,
    const(char)* pool_name,
    ulong auid);
 ...
 * @returns 0 on success, negative error code on failure
 */
int rados_pool_create_with_crush_rule (
    rados_t cluster,
    const(char)* pool_name,
    ubyte crush_rule_num);
 ...
 * @returns 0 on success, negative error code on failure
 */
int rados_pool_create_with_all (
    rados_t cluster,
    const(char)* pool_name,
    ulong auid,
    ubyte crush_rule_num);
...
void rados_object_list_cursor_free (
    rados_ioctx_t io,
    rados_object_list_cursor cur);

int rados_object_list_cursor_cmp (
    rados_ioctx_t io,
    rados_object_list_cursor lhs,
    rados_object_list_cursor rhs);

/**
 * @return the number of items set in the result array
 */
int rados_object_list (
    rados_ioctx_t io,
 ...
 * @returns 0 on success, negative error code on failure
 */
int rados_ioctx_selfmanaged_snap_rollback (
    rados_ioctx_t io,
    const(char)* oid,
    rados_snap_t snapid);
 ...
 */
int rados_ioctx_selfmanaged_snap_set_write_ctx (
    rados_ioctx_t io,
    rados_snap_t seq,
    rados_snap_t* snaps,
    int num_snaps);
 ...
 */
alias void function (rados_completion_t cb, void* arg) rados_callback_t;
 ...
 */
alias void function (ubyte opcode, ulong ver, void* arg) rados_watchcb_t;
 ...
 */
alias void function (void* arg, ulong notify_id, ulong handle, ulong notifier_id, void* data, size_t data_len) rados_watchcb2_t;
 ...
 */
alias void function (void* pre, ulong cookie, int err) rados_watcherrcb_t;
 ...
 * @returns 0 on success, negative error code on failure
 */
int rados_set_alloc_hint (
    rados_ioctx_t io,
    const(char)* o,
    ulong expected_object_size,
    ulong expected_write_size);
 ...
 */
void rados_write_op_cmpxattr (
    rados_write_op_t write_op,
    const(char)* name,
    ubyte comparison_operator,
    const(char)* value,
    size_t value_len);
 ...
 */
void rados_write_op_omap_rm_keys (
    rados_write_op_t write_op,
    const(char*)* keys,
    size_t keys_len);
 ...
 * @param expected_write_size expected size of writes to the object, in bytes
 */
void rados_write_op_set_alloc_hint (
    rados_write_op_t write_op,
    ulong expected_object_size,
    ulong expected_write_size);
 ...
 * @param flags hints about future IO patterns
 */
void rados_write_op_set_alloc_hint2 (
    rados_write_op_t write_op,
    ulong expected_object_size,
    ulong expected_write_size,
    uint flags);
 ...
 * @param flags flags to apply to the entire operation (LIBRADOS_OPERATION_*)
 */
int rados_aio_write_op_operate (
    rados_write_op_t write_op,
    rados_ioctx_t io,
    rados_completion_t completion,
    const(char)* oid,
 ...
 * @param prval where to store the return value from the method
 */
void rados_read_op_exec_user_buf (
    rados_read_op_t read_op,
    const(char)* cls,
    const(char)* method,
    const(char)* in_buf,
 ...
 * @param prval where to store the return value from this action
 */
void rados_read_op_omap_get_vals (
    rados_read_op_t read_op,
 ...
 * @param prval where to store the return value from this action
 */
void rados_read_op_omap_get_keys (
    rados_read_op_t read_op,
    const(char)* start_after,
    ulong max_return,
    rados_omap_iter_t* iter,
 ...
 * @param prval where to store the return value from this action
 */
void rados_read_op_omap_get_vals_by_keys (
    rados_read_op_t read_op,
    const(char*)* keys,
    size_t keys_len,
    rados_omap_iter_t* iter,
 ...
 */
alias void function (void* arg, const(char)* line, const(char)* who, ulong sec, ulong nsec, ulong seq, const(char)* level, const(char)* msg) rados_log_callback_t;
